...
# ╔═╡ 0b3abc51-020b-426b-9394-f48dbb341c4b
begin
	using PlutoUI
	using PlutoUI: Slider # Makie exports Slider, too
end

# ╔═╡ 82acb3fa-6a79-411a-8bfb-2aeaca4863c6
using LinearAlgebra

# ╔═╡ c9af0bc4-4731-41eb-9b80-9ab9d1d5fc6e
using OrdinaryDiffEq

# ╔═╡ f7bc11bd-8812-47a8-9ecf-bc9f72bd5553
using OrdinaryDiffEqCore

# ╔═╡ e629c2ca-3539-42f4-8f80-12e06b5363b6
using GeometricIntegrators: GeometricIntegrators

# ╔═╡ 7bc147fc-f454-42af-941f-3878ccc782ba
using LaTeXStrings

# ╔═╡ 8fbb6827-ec82-440e-9b7c-a5bf523c3d53
begin
	using CairoMakie
...
# ╔═╡ 85791e56-5f7f-11ed-1cd1-b38959c6eb6a
md"""
# 3.2 Ordnungsreduktion

Steife Differentialgleichungen $$u'(t) = f(t, u(t))$$ haben oft eine rechte Seite
$$f$$ mit großer Lipschitz-Konstante $$L$$. Die Lösungen sind aber häufig glatt und
verändern sich nicht stark, zumindest nach einer anfänglichen transienten Periode.
In diesem Fall sind implizite Verfahren zu bevorzugen. Allerdings kann die 
klassische Konvergenz-Theorie nicht voll angewandt werden, da man Schrittweiten
verwenden möchte, die nicht durch $$L^{-1}$$ beschränkt sind. In der Praxis
beobachtet man dann eine Reduktion der Konvergenzordnung, abhängig vom verwendeten
Verfahren.
"""

# ╔═╡ 6dd3fc98-7d89-4723-b123-50309d192ac2
md"""
## Beispiel: Prothero-Robinson-Gleichung

Betrachte

$$u'(t) = \lambda \bigl( u(t) - g(t) \bigr) + g'(t)$$

mit Parameter $\lambda < 0$. Die allgemeine Lösung für einen beliebigen 
Anfangswert ist von der Form

$$u(t) = c \mathrm{e}^{\lambda t} + g(t)$$

mit einem Parameter $c \in \mathbb{R}$.

Hier betrachten wir $g(t) = \cos(t)$ und $u(0) = g(0)$, 
sodass die Lösung $u(t) = g(t)$ unabhängig vom Parameter 
$\lambda$ ist.
"""

# ╔═╡ f8327885-ede9-496d-8cb6-589276f1bab4
md"""
### Gauß-Verfahren

Die Gauss-Kollokationsverfahren zeigen eine Ordnungsreduktion von
$$p = 2s$$ zu $$p = s$$ (gerades $$s$$) bzw. $$p = s + 1$$ (ungerades $$s$$) 
wenn $$h \lambda$$ nicht klein ist. Hier verwenden wir $$\lambda = - 10^6$$.
"""

# ╔═╡ 792d2426-48e7-484c-9fbf-cb14bc13c602
md"""
### Radau-IIA-Verfahren

Man kann zeigen, dass die Radau-IIA-Verfahren für den Fall $$h \to 0$$
und $$z = h \lambda \to \infty$$ einen globalen Fehler der Ordnung
$$\mathcal{O}(z^{-1} h^{s + 1})$$ haben (statt einer Ordnung $$p = 2s - 1$$).
Hier verwenden wir $$\lambda = -10^3$$.
"""

# ╔═╡ b0d52f7f-7df1-466a-be93-cea5c0efa673
md"""
Wenn wir $$\lambda = -10^6$$ verwenden, werden die numerischen Ergebnisse besser (aber nicht unbedingt die Ordnung).
"""

# ╔═╡ a1800c79-1129-4c11-af4b-76be348b3468
md"""
Wenn wir zu $$\lambda = -10^9$$ übergehen, ist die numerische Lösung in vielen
Fällen exakt im Rahmen der Rechengenauigkeit!
"""
...
# ╔═╡ cb08b377-3f70-4351-995a-f1a9cb2fc6f5
space

# ╔═╡ 0bbf7fb1-d702-4881-a00c-8898e54658d3
space

# ╔═╡ 9ab16dd6-79c8-4da8-81bd-5d1a86d2d1e8
space

# ╔═╡ b0af70cb-c6d9-4755-957d-1af2f5767ae5
md"""
#### Installing packages
...
# ╔═╡ cafbe10f-1943-4fc8-b76c-3ec65f81a1d2
# using GeometricIntegratorsDiffEq

# ╔═╡ c35ec370-b2bd-43c7-9da5-412ec4135bb8
import Logging

# ╔═╡ fc09fa67-e908-471f-bb0b-4f6348cc8693
begin
	g(t) = cos(t)
	g′(t) = -sin(t)
end

# ╔═╡ 48518ae7-d1f9-4c43-8f9c-80590900cbca
function prothero_robinson!(du, u, λ, t)
	du[1] = λ * (u[1] - g(t)) + g′(t)
	return nothing
end

# ╔═╡ 454b3d86-c626-4533-98cd-75d03bd8da35
function setup_ode(λ)
	return ODEProblem{true, SciMLBase.FullSpecialize}(prothero_robinson!, [g(0.0)], (0.0, 1.0), λ)
end

# ╔═╡ a66eeb76-c6b4-4cbb-9c05-4d0abc6438c9
function compute_error(ode, alg, dt; kwargs...)
	u = try
		sol = solve(ode, alg; 
	...
	return max(eps(eltype(u)), norm(u .- g(ode.tspan[end])))
end

# ╔═╡ 47dce24a-9043-485e-a33c-2af4457f0f1b
redirect_stdio(stdout = devnull, stderr = devnull) do
	ode = setup_ode(-1.0e3)
	dts = (ode.tspan[end] - ode.tspan[1]) ./ round.(Int, 10.0 .^ range(1, 3, length = 20))

	fig = Figure()
	ax = Axis(fig[1, 1]; xscale = log10, yscale = log10,
			  xlabel = L"Schrittweite $h$", 
			  ylabel = L"Fehler bei $t = 1$")

	alg = RadauIIA5()
	label = L"Radau-IIA, $s = 3$"
	errors = compute_error.((ode,), (alg,), dts)
	scatter!(ax, dts, errors; label = label)

	order = OrdinaryDiffEqCore.alg_order(alg)
	dts_errors = dts[(end ÷ 2):end]
	errors_ideal = dts_errors .^ order; errors_ideal .*= errors[end-1] / errors_ideal[end-1]
	lines!(ax, dts_errors, errors_ideal, label = "Ordnung $order", color = :gray)

	axislegend(ax; position = :lt)
	fig
end

# ╔═╡ ed35e206-174a-4748-9c49-d1290daf4060
redirect_stdio(stdout = devnull, stderr = devnull) do
	ode = setup_ode(-1.0e6)
	dts = (ode.tspan[end] - ode.tspan[1]) ./ round.(Int, 10.0 .^ range(1, 3, length = 20))
...
# ╔═╡ 8b2a9327-b8b2-4392-b603-6eb1e0801641
redirect_stdio(stdout=devnull, stderr=devnull) do
	ode = setup_ode(-1.0e9)
	dts = (ode.tspan[end] - ode.tspan[1]) ./ round.(Int, 10.0 .^ range(1, 3, length = 20))
...
# ╔═╡ 8ee91b60-4c3b-449b-bdf8-2a0ac5b33618
function compute_error_GeometricIntegrators(λ, alg, dt; kwargs...)
	tspan = (0.0, 1.0)
	ode = GeometricIntegrators.ODEProblem(tspan, dt, [g(0.0)]) do du, t, u, p
		prothero_robinson!(du, u, λ, t)
	end
	integrator = GeometricIntegrators.GeometricIntegrator(ode, alg)
	
	u = try
		sol = GeometricIntegrators.integrate(integrator)
		@assert sol.t[end] == last(tspan)
		sol.q[end]
	catch
		return Inf
	end
	return max(eps(eltype(u)), norm(u .- g(tspan[end])))
end

# ╔═╡ bf7c5915-b223-417c-941f-52a7c392ddd3
let
	λ = -1.0e6
	tspan = (0.0, 1.0)
	dts = (tspan[end] - tspan[1]) ./ round.(Int, 10.0 .^ range(1, 3, length = 20))
	...
	alg = GeometricIntegrators.Gauss(2)
	label = L"Gauß, $s = 2$"
	errors = compute_error_GeometricIntegrators.(λ, (alg,), dts)
	scatter!(ax, dts, errors; label = label)

	order = 4
	dts_errors = dts[(end ÷ 2):end]
	errors_ideal = dts_errors .^ order; errors_ideal .*= errors[end] / errors_ideal[end]
	lines!(ax, dts_errors, errors_ideal, label = "Ordnung $order", color = :gray)
	
	order = 2
	dts_errors = dts[1:(end ÷ 2)]
	errors_ideal = dts_errors .^ order; errors_ideal .*= errors[2] / errors_ideal[2]
	lines!(ax, dts_errors, errors_ideal, label = "Ordnung $order", color = :gray)
...
[compat]
CairoMakie = "~0.15.6"
GeometricIntegrators = "~0.15.1"
LaTeXStrings = "~1.4.0"
OrdinaryDiffEq = "~6.102.1"
OrdinaryDiffEqCore = "~1.36.0"
PlutoUI = "~0.7.72"
"""
...
manifest_format = "2.0"
project_hash = "6e320602bb8e655daeb6a6ab0fd403a98e3f62de"
...
git-tree-sha1 = "7af8cd72341980027a3c81800576e7989ce190ca"
...
[[deps.GeometricBase]]
git-tree-sha1 = "7381ca94ea5ea4d535b7c7decc32d2142a9e301d"
uuid = "9a0b12b7-583b-4f04-aa1f-d8551b6addc9"
version = "0.12.4"

[[deps.GeometricEquations]]
deps = ["GeometricBase", "Parameters", "Random"]
git-tree-sha1 = "5c6a38cf068fff676b30925cd15025fddfa85861"
uuid = "c85262ba-a08a-430a-b926-d29770767bf2"
version = "0.20.3"

[[deps.GeometricIntegrators]]
deps = ["CompactBasisFunctions", "Documenter", "ForwardDiff", "GenericLinearAlgebra", "GeometricBase", "GeometricEquations", "GeometricIntegratorsBase", "GeometricSolutions", "LinearAlgebra", "OffsetArrays", "Parameters", "PrettyTables", "ProgressMeter", "QuadratureRules", "Reexport", "RungeKutta", "SharedArrays", "SimpleSolvers", "StaticArrays", "Test"]
git-tree-sha1 = "5c81aafed1ae570da29858aeb7526087bd98a269"
uuid = "dcce2d33-59f6-5b8d-9047-0defad88ae06"
version = "0.15.1"

[[deps.GeometricIntegratorsBase]]
deps = ["GeometricBase", "GeometricEquations", "GeometricSolutions", "LinearAlgebra", "OffsetArrays", "SimpleSolvers", "Unicode"]
git-tree-sha1 = "a642f9e4c74a3986473b661e1a0cb69ab1b18b38"
uuid = "71212ab4-08a2-48eb-a02b-4cbad0e9c44a"
version = "0.1.3"

[[deps.GeometricSolutions]]
deps = ["GeometricBase", "GeometricEquations", "OffsetArrays", "Test"]
git-tree-sha1 = "e8847764179fe0bf0c216b185abdbcd4c7e57630"
...
git-tree-sha1 = "96fbb1a5c700942301cf65d8546690dd3b2a6a79"
...
git-tree-sha1 = "df2ba333529f2fe9318067d4cabf62e19cb97740"
...
[[deps.SimpleSolvers]]
deps = ["Distances", "ForwardDiff", "GeometricBase", "LinearAlgebra", "Printf", "StaticArrays", "Test"]
git-tree-sha1 = "50e5f8194ea93d42fec543a25358dc82743589f1"
