...
# ╔═╡ c67be940-4f55-434f-a6cb-73fa96123f0d
using LinearAlgebra
...
# ╔═╡ ace71dde-792d-4f8d-ba84-007c93301de7
using GenericLinearAlgebra

# ╔═╡ b44a352d-cfe3-45ff-82c7-beecc8396fb4
using NonlinearSolve

# ╔═╡ 85791e56-5f7f-11ed-1cd1-b38959c6eb6a
md"""
# 2.14 Nicht-normale Operatoren

Betrachte lineare, autonome ODEs

$$u'(t) = L u(t)$$

mit halb-beschränktem $L$. Falls $L$ normal ist, d.h., $L L' = L' L$, 
kann die lineare Eigenwertanalyse direkt verwendet werden. Bei nicht-normalen
Operatoren sind Anpassungen notwendig.
"""

# ╔═╡ 802cdea2-dc78-4f45-8e88-398c5e091345
md"""
## Explizite Runge-Kutta-Verfahren dritter Ordnung mit drei Stufen

Die Stabilitätsfunktion solcher Runge-Kutta-Verfahren ist
"""

# ╔═╡ 051b0243-b3c7-4158-801a-52232f350603
R3(z) = one(z) + z + z^2 / 2 + z^3 / 6

# ╔═╡ 99ba94e3-b185-4d5b-b1b7-d850b80c364f
md"""
Das Stabilitätsgebiet ist unten dargestellt.
"""

# ╔═╡ 709ac05e-d620-4db3-b59b-488aed09ae60
let
	N = 500
	x = range(-5, 1, length = N)
	y = range(-3, 3, length = N)
	z = x .+ im .* y'
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"\mathrm{Re}(z)", ylabel = L"\mathrm{Im}(z)")
	heatmap!(ax, x, y, abs.(R3.(z)) .< 1)
	fig
end

# ╔═╡ bf8cc521-0ca4-46f6-932e-7c8dbbf949cc
md"""
Die Länge des Stabilitätsgebietes auf der positiven imaginären Achse ist
$\sqrt{3}$, was man numerisch verifizieren kann:
"""

# ╔═╡ 1bc7ca4d-f767-4df2-9c38-a0918db3d413
let
	f(y, parameters) = abs2(R3(im * y)) - 1
	bounds = (0.5, 3.0)
	prob = IntervalNonlinearProblem(f, bounds)
	sol = solve(prob, Falsi())
	sol.u
end

# ╔═╡ d648ac73-8614-463b-829b-177c75a17221
sqrt(3)

# ╔═╡ f7faad58-8fee-4e62-b27a-754ea15ddc7d
md"""
...
# ╔═╡ 2d7102e6-58d9-445e-84dd-9ffaf62f5dda
L = -[1 2 2; 0 1 2; 0 0 1]

# ╔═╡ 54584299-54f7-4667-9d2d-7291f05f7640
md"""
Dann ist

$$\frac{\mathrm{d}}{\mathrm{d} t} \| u \|^2 = \langle u, L u\rangle + \langle L u, u \rangle = \langle u, (L + L') u \rangle \le 0,$$

denn $L + L'$ ist
"""

# ╔═╡ d3654078-c3cb-43ba-a009-0601b0e5ece2
L + L'

# ╔═╡ 1065ac68-9ec3-4d80-806b-9af88873a11f
md"""
also

$$L + L' = -2 \begin{pmatrix} 1 \\ 1 \\ 1 \end{pmatrix} \begin{pmatrix} 1 & 1 & 1 \end{pmatrix}$$

und damit

$$\langle u, (L + L') u \rangle = -2 (u_1 + u_2 + u_3)^2 \le 0$$

für alle $u \in \mathbb{R}^3$.
"""

# ╔═╡ 1fa0485d-c452-48c3-903e-f30f7664e8fe
md"""
Die Stabilitätsfunktion eines expliziten Runge-Kutta-Verfahrens vierter Ordnung mit vier Stufen ist
"""

# ╔═╡ a3d7cba3-0701-4a2e-a748-2641985863c1
R4(z) = one(z) + z + z^2 / 2 + z^3 / 6 + z^4 / 24

# ╔═╡ b8591f19-b43f-4539-aed6-62d10d6a7ea0
md"""
Man kann zeigen, dass für beliebig kleine $h > 0$

$$\exists u \in \mathbb{R}^3\colon \| R_4(h L) u \|^2 > \| u \|^2.$$

Hier ist zur Veranschaulichung davon die Operatornorm von $R_4(h L)$ dargestellt.
"""

# ╔═╡ 0404d571-570a-4057-ab7e-4ce227721163
let
	dts = big(10.0) .^ range(-15, 0, length=151)
	values = map(dt -> opnorm(R4(dt * L)) - 1, dts)
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"h", ylabel = L"|| R_4(h L) || - 1",
			  yscale = log10)
	lines!(ax, dts, values)
	fig
end
...
# ╔═╡ 09b78af9-42f4-4c37-8f93-3cfa49f671a1
md"""
#### Installing packages
...
manifest_format = "2.0"
project_hash = "80e6be0ae98fb93e67c494c4d216533ab5178c10"
