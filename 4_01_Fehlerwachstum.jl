...
# ╔═╡ 427f8e64-aa53-4457-aa21-84807639a2fd
using LinearAlgebra
...
# ╔═╡ 2bcbe588-637e-4128-9eb5-3b2bcc6b1f49
begin
...
md"""
# 4.1 Fehlerwachstum und die Erhaltung von Invarianten

Wir verwenden verschiedene Verfahren aus 
[OrdinaryDiffEq.jl](https://github.com/SciML/OrdinaryDiffEq.jl), 
um die ODE

$$\begin{equation*}
    \frac{\mathrm{d}}{\mathrm{d} t} \begin{pmatrix} u_1 \\ u_2 \end{pmatrix}
    =
    \frac{1}{u_1^2 + u_2^2} \begin{pmatrix} -u_2 \\ u_1 \end{pmatrix}
\end{equation*}$$

mit Anfangsbedingung $u(0) = (1, 0)^T$ zu lösen:

- `Midpoint()`: explizite Mittelpunktsregel der Ordnung 2; erhält die quadratische Invariante
- `BS3()`: explizites RK-Verfahren von Bogacki & Shampine
- `ImplicitMidpoint()`: implizite Mittelpunktsregel der Ordnung 2; erhält die quadratische Invariante (warum?)
- `Kvaerno3()`: SDIRK-Verfahren von Kvaerno der Ordnung 3

Wir berechnen den Fehler bezüglich der analytischen Lösung im Laufe der Zeit.
"""

# ╔═╡ 588f23e7-7184-4082-a799-acbac893dac6
md"""
Hier erkennt man klar den Vorteil von strukturerhaltenden Verfahren, die ein lineares Fehlerwachstum liefern während die allgemeinen Verfahren zu einem quadratischen Fehlerwachstum führen. Obwohl die beiden Mittelpunktsregeln nur von der Ordnung 2 sind während die anderen beiden Verfahren dritter Ordnung sind, ist der Fehler über lange Zeiträume bei den strukturerhaltenden Verfahren kleiner.

!!! warn "Explizite Mittelpunktsregel"

    Die explizite Mittelpunktsregel wird nicht immer quadratische Invarianten 
	erhalten. Dies hier ist ein spezieller Fall!
"""
...
# ╔═╡ 4e8e9da3-a76a-41e6-9e6f-5d4a4318c9aa
...
# ╔═╡ 833886dd-1652-490a-919b-f0e0df907c04
let
	ode = ODEProblem(rhs!, [1.0, 0.0], (0.0, 250.0))
	dt = 0.1

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"Zeit $t$", ylabel = "Fehler",
			  xscale = log10, yscale = log10)
	
	for alg in [Midpoint(), BS3(), ImplicitMidpoint(), Kvaerno3()]
		sol = solve(ode, alg; adaptive = false, dt)
		errors = [norm(sol.u[i] - [cos(sol.t[i]), sin(sol.t[i])]) 
				  for i in eachindex(sol.u)]
		lines!(ax, sol.t[2:end], errors[2:end]; label = string(nameof(typeof(alg))))
	end
...
manifest_format = "2.0"
project_hash = "2cfd0a6157052f09b8b83022255d2f9bc09f5016"
