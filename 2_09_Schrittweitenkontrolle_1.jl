...
# ╔═╡ 16f1bd6a-570c-49d9-ab55-7bb9029f2281
using LinearAlgebra

# ╔═╡ bcec0bba-2f5d-49d7-a0d3-e4ba50afabc0
using OrdinaryDiffEq

# ╔═╡ 2df98283-6ca4-45d9-bebb-55cb66b057e0
begin
	using CairoMakie
...
# ╔═╡ 72eb6f64-1c57-454c-bc3c-7a8039b8c947
md"""
# 2.9 Schrittweitenkontrolle 1

Hier werden einige Beispiele zur einfachen adaptiven Schrittweitenkontrolle
aus Abschnitt 2.9 der Vorlesung vorgestellt.
"""
...
Arenstorf-Orbits sind geschlosene Trajektorien eines eingeschränkten
Drei-Körper-Problems der Himmelsmechanik. Sie beschreiben beispielsweise
die Bahn eines Satelliten im Gravitationsfeld der Erde und des Mondes,
wobei die Anziehungskraft des Satelliten auf Erde und Mond vernachlässigt
wird.

Die zugehörige ODE im planaren Fall ist

$$\begin{aligned}
x'' &= x + 2 y' - \mu_1 \frac{x + \mu_2}{N_1} - \mu_2 \frac{x - \mu_1}{N_2}, \\
y'' &= y - 2 x' - \mu_1 \frac{y}{N_1} - \mu_2 \frac{y}{N_2}.
\end{aligned}$$

Dabei sind

- ``m_E`` die Masse der Erde
- ``m_M`` die Masse des Mondes
- ``\mu_1 = \frac{m_E}{m_E + m_M} = 0.987722529``
- ``\mu_2 = \frac{m_M}{m_E + m_M} = 0.012277471``

Parameter des Modells. Außerdem sind

- ``N_1 = \bigl( (x + \mu_2)^2 + y^2 \bigr)^{3/2}``
- ``N_2 = \bigl( (x - \mu_1)^2 + y^2 \bigr)^{3/2}``
...
ergibt dich der vierblättrige Arenstorf-Orbit mit Periode

$$T = 17.0652165601579625588917206249.$$
"""

# ╔═╡ 8220033a-7f04-4fe4-8627-45db19a04dcb
md"""
Mit adaptiver Schrittweite kann das Verfahren die Schrittweite ``h`` um
mehrere Größenordnungen anpassen, abhängig von der vorgegeben Toleranz
`tol`.
"""

# ╔═╡ 481a5fc8-008d-4787-b9d7-bc6cdba05e3d
md"""
`tol` = $(@bind tol Slider(10.0 .^ (-8:-5), default = 1.0e-5, show_value = true))
"""

# ╔═╡ af65a0a0-1852-471f-94fe-bc4e38c501f7
md"""
...
# ╔═╡ eb4e354f-5f26-4b96-84a2-3d4ed9a86c95
md"""
## Prothero-Robinson-Problem

Betrachte die ODE

$$u'(t) = \lambda (u(t) - g(t)) + g'(t).$$

Hier wählen wir die Parameter

- ``\lambda = -10^4``
- ``g(t) = \sin(t)``
"""

# ╔═╡ 526a3129-a090-4106-b340-edb856842635
md"""
Wir verwenden zwei verschiedene Schrittweiten-Regler mit der gleichen Toleranz,
den I-Regler (aus Abschnitt 2.9) und einen PI-Regler (aus Abschnitt 2.11). Zunächst 
sehen wir uns die akzeptierten Schrittweiten in einem kleinen Zeitausschnitt an.
"""

# ╔═╡ f9b9b1dc-eb32-4d66-a9f6-b34f721b10c2
md"""
...
Die Norm der Lösung ist eine Invariante (also erhalten), denn

$$\frac{\mathrm{d}}{\mathrm{d}t} \| u(t) \|^2 = 2 u(t) \cdot u'(t) = 2 (- u_1 u_2 + u_2 u_1)(t) = 0.$$

Numerisch ist dies für verschiedene Zeitintegrationsverfahren nicht unbedingt der Fall.
"""

# ╔═╡ 5a0920b5-d5f4-4a17-8f77-0bdef190efa6
md"""
`dt` = $(@bind dt Slider(10.0 .^ (-3:-1), default = 1.0e-2, show_value = true))
"""
...
# ╔═╡ c59a211a-2794-4126-8188-972dd0869a80
space

# ╔═╡ bb5e074e-e08b-4be7-97bd-65b396a123a8
space
...
# ╔═╡ 32e07dcb-7db9-4e75-9bc4-06ee670855d7
md"""
#### Installing packages
...
function arenstorf!(ddu, du, u, parameters, t)
	μ1 = 0.987722529
	μ2 = 0.012277471
	x, y = u
	x′, y′ = du
	N1 = sqrt((x + μ2)^2 + y^2)^3
	N2 = sqrt((x - μ1)^2 + y^2)^3
	
	ddu[1] = x + 2 * y′ - μ1 * (x + μ2) / N1 - μ2 * (x - μ1) / N2
	ddu[2] = y - 2 * x′ - μ1 * y / N1 - μ2 * y / N2
	return nothing
end

# ╔═╡ 603bc4a4-1b91-4d25-afe1-6590b97b5c27
ode_arenstorf = let
	u0 = [0.994, 0.0] # x(0), y(0)
	du0 = [0.0, -2.00158510637908252240537862224] # x'(0), y'(0)
	tspan = (0.0, 17.0652165601579625588917206249)
	SecondOrderODEProblem(arenstorf!, du0, u0, tspan)
end

# ╔═╡ 0a071947-9e23-4218-9436-915f08e1a83c
let
	sol = solve(ode_arenstorf, Tsit5())
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"y")
	t = range(sol.t[begin], sol.t[end], length = 500)
	lines!(sol.(t, idxs = 3), sol.(t, idxs = 4))
	scatter!(ax, [sol.u[1][3]], [sol.u[1][4]]; label = L"x(0), y(0))",
			 color = Makie.wong_colors()[2])
	axislegend(ax)
	fig
end

# ╔═╡ 40c48d9c-6510-46bb-b206-448a68aa0fc2
let
	sol = solve(ode_arenstorf, Tsit5(); 
				controller = IController(), abstol = tol, reltol = tol)
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"Zeit $t$", ylabel = L"Schrittweite $h$",
			  yscale = log10)
	scatter!(ax, sol.t[begin:end-1], diff(sol.t))
	fig
end

# ╔═╡ 9fd56098-1799-41ea-98de-7604a4e61564
function compute_error(; kwargs...)
	sol = solve(ode_arenstorf, Tsit5(); save_everystep = false, kwargs...)
	return norm(sol.u[end] - sol.u[begin]), sol.destats.nf
end

# ╔═╡ 26474fa7-41fd-4928-8ff9-f7e4665e37e3
function compute_error_fixed(dt)
	compute_error(adaptive = false, dt = dt)
end

# ╔═╡ 2b4a2abf-23ad-4872-a0a5-55082ac3c3ae
function compute_error_adaptive(tol)
	compute_error(adaptive = true, abstol = tol, reltol = tol,
				  controller = IController())
end

# ╔═╡ e9f30fd3-7a6b-4379-a108-9d86ff836b3c
let
	fig = Figure()
	ax = Axis(fig[1, 1]; 
			  xlabel = "Anzahl RHS Auswertungen", 
			  ylabel = "Fehler nach einer Periode",
			 xscale = log10, yscale = log10)

	dts = 10.0 .^ range(-4, -2, length = 20)
	errors_rhs_fixed = compute_error_fixed.(dts)
	scatter!(ax, last.(errors_rhs_fixed), first.(errors_rhs_fixed);
			 label = "Uniforme Schrittweiten")
	
	tol = 10.0 .^ range(-10, -2, length = 20)
	errors_rhs_adaptive = compute_error_adaptive.(tol)
	scatter!(ax, last.(errors_rhs_adaptive), first.(errors_rhs_adaptive);
			 label = "Adaptive Schrittweiten")

	axislegend(ax)
	fig
end

# ╔═╡ 4f4dea77-ef23-40a5-bb7a-38d331b23b89
function prothero_robinson(u, λ, t)
	g = sin(t)
	g′ = cos(t)
	return λ * (u - g) + g′
end

# ╔═╡ 254f20f9-821a-4033-9c9b-422c9c1b30e9
begin
	ode_pr = ODEProblem(prothero_robinson, 0.0, (0.0, π / 2), -1.0e4)
	tol_pr = 1.0e-4
	
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"Zeit $t$", ylabel = L"Schrittweite $h$")
	
	sol_I = solve(ode_pr, Tsit5();
				  controller = IController(), abstol = tol_pr, reltol = tol_pr)
	idxs = findall(t -> 0.5 <= t <= 0.6, sol_I.t)
	scatter!(ax, sol_I.t[idxs[1:end-1]], diff(sol_I.t[idxs]); label = "I-Regler")
	
	sol_PI = solve(ode_pr, Tsit5();
				   abstol = tol_pr, reltol = tol_pr)
	idxs = findall(t -> 0.5 <= t <= 0.6, sol_PI.t)
	scatter!(ax, sol_PI.t[idxs[1:end-1]], diff(sol_PI.t[idxs]); label = "PI-Regler")

	axislegend(ax)
	fig
end

# ╔═╡ 4afed832-9718-461a-a085-2d339bbd9d06
md"""
Das oszillierende Verhalten der Schrittweite schlägt sich in der Effizienz
der Verfahren nieder.

- Der I-Regler verwirft $(sol_I.destats.nreject) Schritte
  und nutzt $(sol_I.destats.nf) Funktionsauswertungen.
- Der PI-Regler verwirft nur $(sol_PI.destats.nreject) Schritte
  und nutzt $(sol_PI.destats.nf) Funktionsauswertungen.
"""

# ╔═╡ cedbe79e-bc3a-4dab-b8d4-944d92a05909
function harmonic_osc!(du, u, parameters, t)
	du[1] = -u[2]
	du[2] = u[1]
	return nothing
end

# ╔═╡ 09db09c1-fa1d-49db-ab4c-a4fdb1a44899
let
	ode = ODEProblem(harmonic_osc!, [1.0, 0.0], (0.0, 8π))

	fig = Figure(size = (600, 350)) # default size is (600, 450)
	ax_u = Axis(fig[1, 1]; xlabel = L"u_1", ylabel = L"u_2", aspect = 1)
	ax_U = Axis(fig[1, 2]; xlabel = L"t", ylabel = L"|| u(t) ||", aspect = 1)

	sol = solve(ode, Tsit5(); abstol = 1.0e-6, reltol = 1.0e-6)
	t = range(sol.t[begin], sol.t[end], length = 500)
	lines!(ax_u, sol.(t, idxs = 1), sol.(t, idxs = 2); label = "Referenz")
	lines!(ax_U, sol.t, norm.(sol.u))

	sol = solve(ode, Euler(); dt, adaptive = false)
	lines!(ax_u, sol.(t, idxs = 1), sol.(t, idxs = 2); label = "Exp. Euler")
	lines!(ax_U, sol.t, norm.(sol.u))

	sol = solve(ode, ImplicitEuler(), dt = dt, adaptive = false)
	lines!(ax_u, sol.(t, idxs = 1), sol.(t, idxs = 2); label = "Imp. Euler")
	lines!(ax_U, sol.t, norm.(sol.u))

	# axislegend(ax_u)
	fig[0, 1:2] = Legend(fig, ax_u, framevisible = false, nbanks = 3,
						 tellheight = true)
...
manifest_format = "2.0"
project_hash = "f1808fb0f4718e1f3a81175da6d2c1e3e34ad279"
