...
md"""
# 1.1 Einseitige Lipschitz-Stetigkeit

Wir betrachten die Differentialgleichung

$$u'(t) = \exp(-u(t)).$$

Ein streamplot der rechten Seite $f(t, u) = \exp(-u)$ zeigt, wie sich die Lösungen
...
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t", ylabel = L"u", title = "Strömungsfeld")
	f(t, u) = Point2f(1, exp(-u))
	streamplot!(ax, f, [0, 2], [0, 2])
	xlims!(ax, 0, 2)
	ylims!(ax, 0, 2)
	fig
end

# ╔═╡ 4cd2fec6-e5cd-4000-b224-246ff8ce1895
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t", ylabel = L"u", title = "Lösungen")
	t = range(0, 2, length = 200)
	for u0 in range(0, 2, length = 20)
		u = @. log(t + exp(u0))
		lines!(ax, t, u)
	end
	xlims!(ax, 0, 2)
...
# ╔═╡ c6cb7ecf-7abb-4b00-a743-6c3bc00f424f
md"""
Statt eines Strömungsfeldes kann man sich das Verhalten auch mit Pfeilen veranschaulichen. Hier erkennen wir auch, dass sich Lösungen nicht voneinander
entfernen (und für kleine Werte von $u$ sogar aufeinander zu bewegen).
"""

# ╔═╡ 74d16a77-48a8-4e19-8e3f-a9b0229c46e3
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t", ylabel = L"u")
	f(tu) = Point2f(1, exp(-tu[2])) * 0.3
	x = range(0, 5, length = 10)
	y = range(0, 5, length = 10)
	arrows2d!(ax, x, y, f)
	fig
end

# ╔═╡ 9c0d56e4-8520-42a7-8697-8c3b1fbe354d
md"""
## Positive einseitige Lipschitz-Konstante

Bei einer positiven einseitigen Lipschitz-Konstanten wie bei

$$f(t, u) = u$$

entfernen sich Lösungen voneinander.
"""

# ╔═╡ 1f083467-aafc-4376-904a-6946605cc9c6
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t", ylabel = L"u", title = "Strömungsfeld")
	f(t, u) = Point2f(1, u)
...
# ╔═╡ f4682c42-8156-4f3b-8e41-0f233ebf73a2
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t", ylabel = L"u", title = "Lösungen")
	t = range(0, 2, length = 200)
