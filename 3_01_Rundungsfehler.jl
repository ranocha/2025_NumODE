...
# ╔═╡ bea5dc92-4ae0-4eb5-bf3f-83ccb4d9a5b8
begin
	using PlutoUI
	using PlutoUI: Slider # Makie exports Slider, too
end

# ╔═╡ 3dc65c81-4b62-4141-8d86-23477fde2066
using LinearAlgebra

# ╔═╡ 3855f15c-2696-4214-8940-3b7eee4c7753
using Printf

# ╔═╡ c9af0bc4-4731-41eb-9b80-9ab9d1d5fc6e
using OrdinaryDiffEqTsit5

# ╔═╡ 5484c766-d7dd-47f5-b83e-bb8ae8463825
using OrdinaryDiffEqLowOrderRK

# ╔═╡ 7bc147fc-f454-42af-941f-3878ccc782ba
using LaTeXStrings

# ╔═╡ 4bf80647-daaf-4a72-a1f6-b4668514d9f1
begin
	using CairoMakie
...
# ╔═╡ bbbc2173-d0cb-4aba-b785-7badb890a9a1
using DoubleFloats
...
mit Anfangsbedingung $u(0) = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$
und berechnen den Fehler nach einer Periode.
"""

# ╔═╡ ba045ec4-12c0-4bac-90e8-cd2d19e9490d
let
	algs = [Euler(), Midpoint(), BS3(), RK4(), Tsit5()]
	@bind alg Select(map(alg -> alg => nameof(typeof(alg)), algs))
end

# ╔═╡ cbafd230-6425-462e-b327-c666201f8e54
@bind FloatType Select([Float32, Float64, Double64])
...
# ╔═╡ 88cd840e-a837-41ec-8ffe-a8c8931212ad
space
...
# ╔═╡ df679447-5026-40e2-86f5-50268a80a534
md"""
#### Installing packages
...
# ╔═╡ 64e520e3-d25d-4bc4-b3f8-b9e340d595c2
...
# ╔═╡ f3cb3b62-8be6-4d82-afe3-61da60ba3cd2
function compute_error(alg, dt)
	FloatType = typeof(dt)
	u0 = FloatType[1, 0]
	tspan = (zero(FloatType), 2 * FloatType(π))
	ode = ODEProblem(f!, u0, tspan)
	sol = solve(ode, alg; 
				adaptive = false, dt = dt, 
				save_everystep = false, maxiters = 10^7)
	return norm(sol.u[end] - sol.u[1])
end

# ╔═╡ 39a1aa32-c3f2-4bc7-b2a1-47cb933e9bd0
begin
	dts = FloatType(10) .^ range(FloatType(-6), FloatType(1), length=10)
	err = compute_error.(Ref(alg), dts)
	fig = Figure()
	ax = Axis(fig[1, 1]; xscale = log10, yscale = log10,
			  xlabel = L"h", ylabel = "Fehler")
	scatter!(ax, dts, err)
	fig
end

# ╔═╡ fea13543-129c-4793-9b99-d37a7428da1f
md"""
Minimaler Fehler:
$(@sprintf("%.2e", minimum(err)))
"""
...
manifest_format = "2.0"
project_hash = "0dcbf16b79ad57cf2dbd90fc586117409709a20f"
