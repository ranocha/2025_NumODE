...
md"""
# 2.10 Steife Systeme 1

Es gibt keine zufriedenstellende Definition "steifer Differentialgleichungen".
Typischerweise sind diese dadurch charakterisiert, dass explizite Verfahren
nicht gut funktionieren, implizite Verfahren allerdings schon.
"""

# ╔═╡ 6dd3fc98-7d89-4723-b123-50309d192ac2
md"""
## Beispiel: Lineare Systeme

Betrachte die ODEs

$$u'(t) = \underbrace{\begin{pmatrix} -2 & 1 \\ 1 & -2 \end{pmatrix}}_{=: A_1} u(t) + \begin{pmatrix} 2 \sin(t) \\ 2 \bigl( \cos(t) - \sin(t) \bigr) \end{pmatrix}$$

und

$$u'(t) = \underbrace{\begin{pmatrix} -2 & 1 \\ 998 & -999 \end{pmatrix}}_{=: A_2} u(t) + \begin{pmatrix} 2 \sin(t) \\ 999 \bigl( \cos(t) - \sin(t) \bigr) \end{pmatrix}$$

mit dem Anfangswert

$$u(0) = \begin{pmatrix} 2 \\ 3 \end{pmatrix}.$$

Beide besitzen die gleiche Lösung

$$u(t) = 2 \mathrm{e}^{-t} \begin{pmatrix} 1 \\ 1 \end{pmatrix} + \begin{pmatrix} \sin(t) \\ \cos(t) \end{pmatrix}.$$

Numerische Verfahren verhalten sich allerdings deutlich unterschiedlich.
"""

# ╔═╡ 26f4aeeb-6841-463e-9451-8313f2b8c2cd
md"""
Expliziter Algorithmus:
"""

# ╔═╡ 7682eaa9-e4a1-4a2a-8f66-6a75b65b0036
let
	list = [Tsit5(), DP5(), BS3(), Vern7(), Vern9()]
	@bind alg_exp Select([alg => nameof(typeof(alg)) for alg in list])
end

# ╔═╡ d5e97a70-5045-433e-999f-3a15043271db
md"""
Impliziter Algorithmus:
"""

# ╔═╡ 69e4eed9-7bbc-4a72-8192-3ed608325120
let
	list = [RadauIIA5(), Rodas5P(), Rodas4(), KenCarp4(), Rosenbrock23(), TRBDF2(), QNDF()]
	@bind alg_imp Select([alg => nameof(typeof(alg)) for alg in list])
end

# ╔═╡ 9dd272fa-992e-436b-8f6f-787fe0656fea
md"""
...
Hier betrachten wir $g(t) = \mathrm{e}^{-t}$ und $u(0) = g(0)$, 
sodass die Lösung $u(t) = \mathrm{e}^{-t}$ unabhängig vom Parameter 
$\lambda$ ist.
"""

# ╔═╡ 3b7db9ed-5435-42fe-a9af-4838dd310554
md"""
`dt` = $(@bind dt Slider(10.0 .^ (-5:-1), default = 1.0e-1, show_value = true))
"""

# ╔═╡ a072ff24-2a5b-424a-bae8-de37da491da8
md"""
`λ` = $(@bind λ Select([-1.0e1, -1.0e4]))
"""
...
function linear1!(du, u, parameters, t)
	...
	du[1] = -2 * u1 + u2 + 2 * si
	du[2] = u1 - 2 * u2 + 2 * (co - si)
	return nothing
end

# ╔═╡ 12db05b5-d450-47dc-b51c-473efde5cccb
function linear2!(du, u, parameters, t)
	u1, u2 = u
	si, co = sincos(t)
	
	du[1] = -2 * u1 + u2 + 2 * si
	du[2] = 998 * u1 - 999 * u2 + 2 * (co - si)
	return nothing
end

# ╔═╡ ee211617-2ddb-46f0-90cd-eb1fcaa1c9e3
begin
	u0 = [2, 3]
	tspan = (0.0, 10.0)
	ode1 = ODEProblem(linear1!, u0, tspan)
	ode2 = ODEProblem(linear2!, u0, tspan)

	sol1_exp = solve(ode1, alg_exp)
	sol1_imp = solve(ode1, alg_imp)

	sol2_exp = solve(ode2, alg_exp)
	sol2_imp = solve(ode2, alg_imp)

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t")
	t = range(sol1_exp.t[begin], sol1_exp.t[end], length = 500)
	lines!(ax, t, sol1_exp.(t, idxs = 1); label = L"u_1")
	lines!(ax, t, sol1_exp.(t, idxs = 2); label = L"u_2")
	axislegend(ax)
	fig
end

# ╔═╡ cefff8ae-cb50-4dd0-80dd-27c9cd6dec55
md"""

|       |               | $(nameof(typeof(alg_exp)))  | $(nameof(typeof(alg_imp)))
|-------|---------------|-----------------------------|--------------------------
| ODE 1 | Schritte akz. | $(sol1_exp.destats.naccept) | $(sol1_imp.destats.naccept)
|       | Schritte ver. | $(sol1_exp.destats.nreject) | $(sol1_imp.destats.nreject)
|       | RHS Ausw.     | $(sol1_exp.destats.nf)      | $(sol1_imp.destats.nf)
|       | Lin. Lsg.     | $(sol1_exp.destats.nsolve)  | $(sol1_imp.destats.nsolve)
| ODE 2 | Schritte akz. | $(sol2_exp.destats.naccept) | $(sol2_imp.destats.naccept)
|       | Schritte ver. | $(sol2_exp.destats.nreject) | $(sol2_imp.destats.nreject)
|       | RHS Ausw.     | $(sol2_exp.destats.nf)      | $(sol2_imp.destats.nf)
|       | Lin. Lsg.     | $(sol2_exp.destats.nsolve)  | $(sol2_imp.destats.nsolve)
"""

# ╔═╡ 81db4ff3-d982-4256-970c-90cdedb52cdd
function prothero_robinson!(du, u, parameters, t)
	λ = parameters[1]

	du[1] = λ * (u[1] - exp(-t)) - exp(-t)
	return nothing
end

# ╔═╡ f0849cf5-8f68-49ba-b217-a69da3cc1ed5
begin
	ode_prothero_robinson = ODEProblem(prothero_robinson!, [1.0], (0.0, 1.0), [λ])

	alg_exp1 = Euler()
	alg_imp1 = ImplicitEuler()

	sol_prothero_robinson_exp = solve(ode_prothero_robinson, alg_exp1, dt=dt, adaptive=false, save_everystep=false)
	sol_prothero_robinson_imp = solve(ode_prothero_robinson, alg_imp1, dt=dt, adaptive=false, save_everystep=false)

	u_ana = [exp(-last(ode_prothero_robinson.tspan))]
	err_exp = norm(sol_prothero_robinson_exp.u[end] - u_ana)
	err_imp = norm(sol_prothero_robinson_imp.u[end] - u_ana)

	nothing
end

# ╔═╡ 3bed37bc-cea6-4433-9185-d25f50443a48
md"""

|         | $(nameof(typeof(alg_exp1)))    | $(nameof(typeof(alg_imp1)))
|---------|--------------------------------|----------------------------
| Schritte| $(sol_prothero_robinson_exp.destats.naccept) | $(sol_prothero_robinson_imp.destats.naccept)
| Endzeit | $(@sprintf("%.2f", sol_prothero_robinson_exp.t[end])) | $(@sprintf("%.2f", sol_prothero_robinson_imp.t[end]))
| Fehler  | $(@sprintf("%.2e", err_exp)) | $(@sprintf("%.2e", err_imp))
"""
...
manifest_format = "2.0"
project_hash = "ef3bdcc7607b0bfce7c05c36ee99ae7aad20d1c8"
