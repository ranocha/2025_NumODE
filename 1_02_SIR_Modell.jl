...
Das SIR-Modell ist ein sehr simpler Zugang zur Modellierung von Epidemien.
Die Bevölkerung wird dabei in drei Gruppen eingeteilt:

- `S`: susceptible, d.h., anfällige Personen,
- `I`: infizierte Personen,
- `R`: removed, d.h., aus dem Modell entfernte Personen.

Im günstigsten Fall ist "entfernt" dabei als "wieder gesund und immun" zu
interpretieren.

Heutzutage finden Sie viele Motivationen für solche Modelle,
etwa in dem Buch
["So einfach ist Mathematik - Gewöhnliche Differentialgleichungen
für Anwender"](https://doi.org/10.1007/978-3-662-64831-5)
von Dirk Langemann - eine nette Einführung in ODEs für
Ingenieurswissenschaften, also mit vielen Motivationen, Veranschaulichungen und
Beispielen.

Die zugehörige ODE ist

$$\begin{aligned}
S' &= -\alpha S I, \\
I' &= \alpha S I - \varrho I, \\
R' &= \varrho I.
\end{aligned}$$

Dabei sind die Konstanten $\alpha$ (Ansteckungsrate)
und $\varrho$ (Immunisierungsrate) Parameter des Modells.
"""
...
md"""
α = $(@bind α Slider(-0.5:0.01:2.0, default=1.0, show_value=true))

ϱ = $(@bind ϱ Slider(0.0:0.01:2.0, default=0.1, show_value=true))
"""
...
begin
	u0 = [0.99, 0.01, 0.0] # 99% Gesunde, 1% Erkrankte
	tspan = (0.0, 50.0)
	parameters = [α, ϱ]
	ode = ODEProblem(f!, u0, tspan, parameters)
	sol = solve(ode, Tsit5())

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"Zeit $t$")
	t = range(tspan..., length = 500)
	lines!(ax, t, sol.(t, idxs = 1); label = L"S(t)")
	lines!(ax, t, sol.(t, idxs = 2); label = L"I(t)")
	lines!(ax, t, sol.(t, idxs = 3); label = L"R(t)")
...
manifest_format = "2.0"
project_hash = "e97ccb1a38e9666cb21da5c2eb1f0cc3c4c8e151"
