...
md"""
# 2.13 Steife Systeme 2

Sowohl Gauß- als auch Radau-IIA-Verfahren sind $A$-stabil. Sie verhalten sich
bei Anwendung auf steife Differentialgleichungen aber trotzdem sehr unterschiedlich!
"""
...
$$u'(t) = -2000 \bigl( u(t) - \cos(t) \bigr)$$

mit dem Anfangswert

$$u(0) = 0.$$

Die Lösung hat eine stark gedämpfte Komponente, die schnell abklingt.
"""

# ╔═╡ 3b7db9ed-5435-42fe-a9af-4838dd310554
md"""
`dt` = $(@bind dt Slider(10.0 .^ (-4:-1), default = 1.0e-2, show_value = true))
"""

# ╔═╡ d6e67960-90ea-4827-861b-65b626d0ef15
let
	ode = ODEProblem((u, λ, t) -> -2000 * (u - cos(t)), 0.0, (0.0, π))

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"t", ylabel = L"u")
	
	sol = solve(ode, ImplicitMidpoint(); adaptive = false, dt)
	scatter!(ax, sol.t, sol.u, label = "Gauß(2)")
	
	sol = solve(ode, ImplicitEuler(); adaptive = false, dt)
	scatter!(ax, sol.t, sol.u, label = "RadauIIA(1)")
...
manifest_format = "2.0"
project_hash = "27a9556dddf8b4eb5c261093b0831f3fcd20b243"
